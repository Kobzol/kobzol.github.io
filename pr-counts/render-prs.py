...
def load_data(path: str) -> tuple[dict, dict]:
    ...
    return data["pull-requests"], data["reviews"]
...
def render_stats(prs: dict, reviewed_prs: dict, mode: str) -> str:
...
def render_post(prs: dict, reviewed_prs: dict, post_path: str, mode: str):
    with open(post_path) as f:
        post = f.read()

    post = insert_after_marker(post, PR_STATS_MARKER, render_stats(prs, reviewed_prs, mode))
...
def main():
    ...
    prs, reviewed_prs = load_data(args.path)
    ...
    render_post(prs, reviewed_prs, args.post_path, args.mode)
