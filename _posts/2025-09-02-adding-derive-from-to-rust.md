...
However, this approach cannot be currently used with all "standard" traits from the standard library.
One of those traits is the [`From`](https://doc.rust-lang.org/std/convert/trait.From.html) trait,
which is used to convert between values of two types. Sometimes, it is not a good idea to
implement `From` when using a newtype (more on that later), but in other cases, it's perfectly fine,
and it makes it easier to go from a value of the inner field to a value of the newtype (especially
...
At the time of writing of this blog post, it has exactly one hundred emoji reactions, which actually
...
It is one of those things that fill a gap in Rust; it allows combining two features that were working
separately before (the `From` trait) and deriving standard library traits, but couldn't be combined.
That being said, this analogy may be a bit hard to sell, as e.g. the `async fn in trait` feature
could also be described in a similar way, but that requires a lot of language considerations.
...
3. Generate a `From` impl from all the values at once, i.e. `From<(u32, bool)`

I think that option 3) would be quite unintuitive, and it would run into an issue for the most common
use-case where you have a single field - should we generate `From(field)` (and thus break consistency
with other field counts) or should we generate `From((field,))`, i.e. generate the struct from a tuple
of size one (which is quite unergonomic)? This also wouldn't really work for structs with named fields.
...
when deriving `Default`, so there is precedent for doing something like this. However, since I wanted
to keep this RFC minimal in order to improve its chances of being accepted without falling into
deep "design holes", I didn't include it in the RFC and left it only as a future possibility.

And thus I chose to go with option 1), which means that you can currently only use `#[derive(From)]` on
...
design for it, that will be extensible in the future, and won't close doors to a better solution for
the same problem?
...
If `rustc` generated anything else than this for `#[derive(From)] struct Foo(u32)`:
...
Then I think that the feature would be wrong. And thus I wanted to get my minimalistic RFC out the door,
so that we can start testing the feature in the wild, and mainly so that it can start providing
value to Rust users.

It should be possible to lift the limitations (such as only allowing it on structs with a single field)
and add extensions (e.g. the `#[from]` attribute or `#[derive(Into)]`) of this feature
...
nested in the newtype. A typical example is something like `struct Email(String)`. An `Email` is backed
...
It seemed to me that people who expressed concerns about whether we should make implementation
...
I personally think that `rustc` should not dictate what is and what isn't "good program design",
of course as long as makes sure that it is *sound* (in the sense of memory/data race/etc. safety).
Obviously, Rust(c) already makes some strong decisions about program design that you cannot easily
work around, for example it does not allow you to use inheritance. But in this specific case,
you can already implement `From` by hand, so forbidding users from deriving this specific trait, even
though it is technically possible, would seem to be a bit silly to me.

## Implementing the feature in rustc

Normally, after a RFC is accepted, we have to wait until *someone* actually implements it (which is
...
were `derive`able, so I had to make some changes to the `derive` infrastructure to make it work. It
was also a chance to work with [Nick Nethercote](http://github.com/nnethercote) again, which is always awesome.
...
name resolution to understand what `Trait` is. One option is that the trait is resolved to an existing
...
The code used a glob import to import a macro named `From`. After my PR, this module also got another
named `From` (the one from the standard library) imported with a glob import, which caused a new
ambiguity. In other words, my change broke existing (stable) code, even without people using the new
feature. That is obviously Very Bad.
...
But having two macros imported *both* with a glob import (with one coming from the prelude) causes
a problem.

## How to use it

To experiment with this feature, update to a recent nightly release of the compiler, and try to
simply `#[derive(From)]` on a struct of your choice. Note that it will only work on structs with
exactly a single field, as mentioned above! You will also have to import the macro explicitly,
